...
def generateHeaderFile(execList, fileName):
	...
	for x in execList:
		temp = getHexDump(x)
		prog_lengths.append(len(temp))
		headerFile.write("new unsigned char[");
		headerFile.write(str(len(temp.split(','))));
		#headerFile.write(str(prog_lengths[-1]))
		headerFile.write("]{");
		headerFile.write(temp);
		headerFile.seek(-1,os.SEEK_END)
		headerFile.truncate()
		headerFile.seek(0,2)
		headerFile.write("},");
...
def compileFile(binderCppFileName, execName):
	...
	p = Popen(["g++","-std=gnu++11","-o",execName,binderCppFileName],stdout=PIPE)
